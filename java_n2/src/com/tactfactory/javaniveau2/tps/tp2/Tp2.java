...
//     1 : Le nombre d'utilisateur
		...
		// 2 : Le nombre d'utilisateur dans la première lettre du prénom commence par
		// "a"
		long nbUsersA = users.stream().filter(x -> x.getFirstname().charAt(0) == 'a').count();
		System.out.println(String.format("%s %d",
				"Nombres d'utilisateur dans la première lettre du prénom commence par \"a\"", nbUsersA));

		// 3 : Le nombre d'utilisateur dont le nom et le prénom contienent exactement 5
		// caractères
		...
		System.out.println(String.format("%s %d",
				"Les utilisateurs dont le full name contient entre 13 et 16 caractères", nbUsersFullname1316));
		...
		// 8 : La marque possédant le plus de voiture avec le nombre de voiture
		// possédé
